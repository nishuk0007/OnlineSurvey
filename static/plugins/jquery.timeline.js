	...
	$.widget( "grantorino.timeline" , {
		...
		_tpl_event: ['<li class="tl-item">',
						'<div class="tl-wrap {{class}}">',
							...
							'<div class="img_div">',
							'<img width="35" class="media-object rounded" src={{img_src}}  alt="...">',
							'</div>',
		...
		update: function(event_data){
			this.element.find('ul.timeline li').remove();
			this.options.data = event_data;
			this._buildContainer();
			this._buildTimeline();
		},
		...
		_render_event: function(data){
			...
			event_html = event_html.replace('{{img_src}}', data.image);
